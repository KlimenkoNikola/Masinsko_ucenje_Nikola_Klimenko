...
class MontenegroEVCS:
    ...
    def _calculate_distance(self, loc1: Tuple[float, float], 
        ...
        # Korekcija za planinsku geografiju Crne Gore (~1.15)
        # Pošto rute moraju da idu oko planina
        road_factor = 1.15
    ...
    def _calculate_fitness(self, chargers: List[Tuple[int, int]]) -> float:
        ...
        # 2. MINIMALNA DISTANCA IZMEĐU PUNJAČA (sprečava duplikiranje)
        spacing_penalty = 0
        ...
        # 3. RAVNOMERNA DISTRIBUCIJA (ne smeju biti sve na istom mjestu)
        if len(chargers) > 1:
        ...
        # 5. OGRANIČENJE: Svaki grad mora imati bar jedan punjač u razumnoj distanci
        coverage_constraint = 0
        for city in self.cities:
            min_dist = min([self._calculate_distance((city.latitude, city.longitude), loc) 
                           for loc in charger_locations])
            if min_dist > 50:  # Preklop se grad nalazi dalje od 50 km
    ...
    def _mutate(self, solution: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
        ...
        if random.random() < 0.4 and len(mutated) > 0:
